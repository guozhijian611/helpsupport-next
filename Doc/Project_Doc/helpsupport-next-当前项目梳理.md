...
- 当前根目录没有 `uniapp/`，但 `deploy/deploy.sh` 和 `deploy/docker.sh` 仍保留了 `uniapp H5` 相关打包变量和提示，这是父框架脚本残留，不代表当前仓库已经恢复了 H5 端源码。
- `Doc/Project_Doc/review/` 和 `Doc/Project_Doc/help-rebuild-development-guide.md` 主要是历史资料，不是当前运行事实的唯一来源。
...
```text
Doc/OpenAPI/help/openapi.yaml
```

`Doc/OpenAPI/help/README.md` 中已说明它来自：
...
- 对外接口变更后同步刷新 APIDOC 注解，并更新 `Doc/OpenAPI/help/openapi.yaml`。
...
- 当前 `deploy/deploy.sh`、`deploy/docker.sh` 仍带有 `uniapp H5` 的父框架脚本逻辑。
...
- 需要发布前，先确认本次是否仅涉及：
  ...
  - `flutter_app`
  - `Database`
- 如果要继续使用 `deploy/` 下的发布脚本，建议先核对目标是否仍然是当前项目需要的发布边界。

## 11. 推荐阅读顺序

1. 先读本文，理解当前仓库现实边界。
2. 再看 `Doc/Project_Doc/README.md` 了解主文档与历史资料分工。
3. 做后端开发时，看 `server/plugin/help`、`Database/migrations/`、`Doc/OpenAPI/help`。
