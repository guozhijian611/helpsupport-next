...
- **AI 管理 / 模型中心 / 在线模型**：维护可被 App 选用的在线文本模型，以及 realtime / ASR / TTS 配置。`asr` / `tts` 是非实时 HTTP 通道（`/v1/audio/transcriptions`、`/v1/audio/speech`），不要填 `wss://.../realtime`。
- **AI 管理 / 模型中心 / 本地模型目录、本地模型提示词**：端侧模型与旧本地提示词。
- **AI 管理 / 提示词 / 用户改写提示词**：会员改过的提示词巡检，`chat_mode` 下拉从角色目录动态加载。
- **AI 管理 / 会话记录 / 聊天会话、聊天记录**：用户会话与消息。
- **AI 管理 / 模型测试 / 对话测试、ASR测试、TTS测试、实时测试**：分别打开 SAIAI 试聊台、在线转写、在线合成和实时测试页，不承担 App 产品角色。对话分组 CRUD 不再作为测试入口。
