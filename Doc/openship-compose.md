...
当前 Compose 面向测试服务器，Phinx 增量迁移默认开启。`app` 入口会先输出迁移状态，再执行所有待处理迁移：

```dotenv
B8_RUN_MIGRATIONS=1
```

全新数据库的一整批迁移可能存在前后依赖，Phinx `--dry-run` 不会真实创建前序表，因此不适合作为容器启动的强制前置步骤。生产环境仍应先关闭自动迁移，并在已有数据库上单独执行 `status`、`--dry-run`，确认备份、目标环境和回滚窗口后再执行真实迁移。
