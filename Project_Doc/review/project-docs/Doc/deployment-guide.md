...
- 服务器别名：`b8org`
- 远端目录：`/www/wwwroot/helpsupport`
- Webman 端口：`8787`
- 同步内容：`server/`、admin 构建产物、H5 构建产物、数据库（显式开启）
- H5 目标目录：`/www/wwwroot/helpsupport/server/public/h5`
- 数据库同步：默认关闭
- Webman 禁用函数检查：默认开启
...
H5_PUBLIC_BASE=/h5/ ./deploy.sh
SYNC_DB=1 ./deploy.sh
CHECK_WEBMAN_DISABLED_FUNCTIONS=0 ./deploy.sh
REMOTE_ROOT=/www/wwwroot/其他项目目录 ./deploy.sh
```

`BUILD_H5=1` 会在 `helpsupport-frontend/` 执行 `pnpm build:h5:prod`，并把 `dist/build/h5/` 同步到远端 `server/public/h5/`。
...
```bash
cd /www/wwwroot/helpsupport/server
