<?php

namespace plugin\help\app\admin\controller\gamification;

use plugin\help\app\admin\logic\gamification\SaMemberBadgeLogic;
use plugin\help\app\admin\validate\gamification\SaMemberBadgeValidate;
use plugin\saiadmin\basic\BaseController;
use plugin\saiadmin\service\Permission;
use support\Request;
use support\Response;

/**
 * 会员徽章记录控制器
 */
class SaMemberBadgeController extends BaseController
{
    public function __construct()
    {
        $this->logic = new SaMemberBadgeLogic();
        $this->validate = new SaMemberBadgeValidate();
        parent::__construct();
    }

    #[Permission('会员徽章记录列表', 'help:gamification:badge:index')]
    public function index(Request $request): Response
    {
        $where = $request->more([
            ['member_id', ''],
            ['badge_code', ''],
            ['badge_name', ''],
            ['source_type', ''],
            ['status', ''],
        ]);

        return $this->success($this->logic->getList($this->logic->search($where)));
    }

    #[Permission('会员徽章记录读取', 'help:gamification:badge:read')]
    public function read(Request $request): Response
    {
        $model = $this->logic->read($request->input('id', ''));
        if (!$model) {
            return $this->fail('未查找到信息');
        }

        return $this->success(is_array($model) ? $model : $model->toArray());
    }

    #[Permission('会员徽章记录添加', 'help:gamification:badge:save')]
    public function save(Request $request): Response
    {
        $data = $this->logic->prepareAwardData($request->post());
        $this->validate('save', $data);
        $result = $this->logic->add($data);

        return $result ? $this->success('添加成功') : $this->fail('添加失败');
    }

    #[Permission('会员徽章记录修改', 'help:gamification:badge:update')]
    public function update(Request $request): Response
    {
        $data = $this->logic->prepareAwardData($request->post());
        $this->validate('update', $data);
        $result = $this->logic->edit($data['id'], $data);

        return $result ? $this->success('修改成功') : $this->fail('修改失败');
    }

    #[Permission('会员徽章记录删除', 'help:gamification:badge:destroy')]
    public function destroy(Request $request): Response
    {
        $ids = $request->post('ids', '');
        if (empty($ids)) {
            return $this->fail('请选择要删除的数据');
        }
        $result = $this->logic->destroy($ids);

        return $result ? $this->success('删除成功') : $this->fail('删除失败');
    }
}
