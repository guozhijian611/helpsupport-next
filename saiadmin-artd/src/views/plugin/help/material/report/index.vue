    ...
    <ElCard class="art-table-card" shadow="never">
      <ArtTableHeader v-model:columns="columnChecks" :loading="loading" @refresh="refreshData">
        <template #left>
          <ElSpace wrap>
            <ElButton
              v-permission="'help:material:report:save'"
            ...
            <ElButton
              v-permission="'help:material:report:destroy'"
      ...
      >
        ...
        <template #operation="{ row }">
          <div class="flex gap-2">
            ...
            <ElButton
              v-permission="'help:material:report:update'"
              size="small"
              type="primary"
            ...
            <ElButton
              ...
              v-permission="'help:material:report:handle'"
              size="small"
              type="success"
...
<script setup lang="ts">
  ...
  const {
    ...
    core: {
      ...
      columnsFactory: () => [
        ...
        { prop: 'operation', label: '操作', width: 250, fixed: 'right', useSlot: true }
      ]
    }
  })
  ...
  const handleReport = async (row: Record<string, any>, handleStatus: number) => {
    const result = await ElMessageBox.prompt(
      `请输入${handleStatus === 1 ? '处理备注，将同步隐藏被举报素材/评论' : '忽略备注'}`,
