...
- Android：运行 `./tool/build_android_llama.sh`，脚本会按 `llama_cpp_dart` 绑定对应的 `llama.cpp` 提交构建 CPU-only `libmtmd.so`、`libllama.so`、`libggml*.so`，并复制 NDK 运行时 `libc++_shared.so`、`libomp.so` 到 `android/app/src/main/jniLibs/<abi>/`。
