...
- 如果检测到其他已修改、已暂存或未跟踪的 Git 变更，除非用户明确要求处理，否则不要查看、修改、回退、取消暂存或提交这些无关变更，只处理并提交自己本次任务涉及的文件。
- 提交前只暂存本次任务相关文件；如果索引里已有无关暂存内容，使用限定路径或 `git commit --only <本次任务文件>` 等方式确保本次 commit 只包含自己处理的这部分。
...
- 如果已有其他文件处于 staged 状态，不要为了本次提交执行全局 `git reset` 或取消暂存；应使用路径限定提交，只提交本次任务文件，保留原有 staged 状态给用户继续处理。
